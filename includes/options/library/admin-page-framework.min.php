<?php 
/**
 *  Admin Page Framework - Minified Version v3.4.6 by Michael Uno 
...
final class AdminPageFramework_Bootstrap {
    function __construct($sLibraryPath) {
        ...
        if (!AdminPageFramework_Registry::$bIsMinifiedVersion) {
            ...
            new AdminPageFramework_RegisterClasses(isset($aClassFiles) ? '' : AdminPageFramework_Registry::$sDirPath, array(), isset($aClassFiles) ? $aClassFiles : array());
        }
    }
}
...
abstract class AdminPageFramework_TaxonomyField extends AdminPageFramework_Factory {
    ...
    public function _replyToSetColumnCell($vValue, $sColumnSlug, $sTermID) {
        ...
        $sCellHTML = $this->oUtil->addAndApplyFilter($this, "cell_{$this->oProp->sClassName}_{$sColumnSlug}", $sCellHTML, $sTermID);
        echo $sCellHTML;
    }
...
class AdminPageFramework_Debug extends AdminPageFramework_WPUtility {
    ...
    static public function log($vValue, $sFilePath = null) {
        ...
        $_sHeading = date("Y/m/d H:i:s", $_nNow) . '.' . $_nMicroseconds . ' ' . $_sElapsed . '.' . $_sElapsedFloat . ' ' . $_iPageLoadID . ' ' . AdminPageFramework_Registry::Version . (AdminPageFramework_Registry::$bIsMinifiedVersion ? '.min' : '') . ' ' . "{$_sCallerClasss}::{$_sCallerFunction} " . current_filter() . ' ' . self::getCurrentURL() . ' ';
...
class AdminPageFramework_ExportOptions extends AdminPageFramework_CustomSubmitFields {
    ...
    public function doExport($vData, $sFileName = null, $sFormatType = null) {
        ...
        switch (strtolower($sFormatType)) {
            case 'text':
                if (is_array($vData) || is_object($vData)) die(AdminPageFramework_Debug::getArray($vData, null, false));
                die($vData);
            case 'json':
...
class AdminPageFramework_Message {
    ...
    public static function getInstance($sTextDomain = 'admin-page-framework') {
        $_oInstance = isset($_aInstancesByTextDomain[$sTextDomain]) && ($_aInstancesByTextDomain[$sTextDomain] instanceof AdminPageFramework_Message) ? $_aInstancesByTextDomain[$sTextDomain] : new AdminPageFramework_Message($sTextDomain);
